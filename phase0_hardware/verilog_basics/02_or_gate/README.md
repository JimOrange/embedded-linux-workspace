...
> 实验目标：实现一个二输入或门。当任意一个输入按键按下时，LED 点亮。
...
- **烧录时**（`SIM` 未定义）：执行负逻辑适配，实现“任意按键按下时 LED 亮”
...
|------|----------|----------|
| 两个按键都松开 | LED 灭 | ✅ 通过 |
| 只按左侧 KEY1（a） | LED 亮 | ✅ 通过 |
| 只按右侧 KEY0（b） | LED 亮 | ✅ 通过 |
| 左右同时按下 | LED 亮 | ✅ 通过 |


## 仿真波形

![仿真波形](./or_gate_sim_waveform.png)

*图：或门功能仿真波形（正逻辑）。图中依次覆盖 4 种输入组合，验证了标准或门功能：只要 a 或 b 为 1，输出即为 1。*

## RTL 视图

![RTL视图](./or_gate_rtl.png)
